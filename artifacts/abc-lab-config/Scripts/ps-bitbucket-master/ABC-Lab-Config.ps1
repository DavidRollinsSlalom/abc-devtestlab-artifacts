...
$msgBoxInput = [System.Windows.MessageBox]::Show('Would you like to configure the lab for Java?', 'Platform Select', 'YesNo', 'Information')
switch ($msgBoxInput) {
    ...
    'No' {
        $msgBoxInput = [System.Windows.MessageBox]::Show('Would you like to configure the lab for .Net Core?', 'Platform Select', 'YesNo', 'Information')
        switch ($msgBoxInput) {
            'Yes' {
                Set-BitBucketServer -Url "https://api.bitbucket.org"
                $BitBucketCred = Get-Credential -Message "Please enter the username and password for your BitBucket Account provided by the lab instructor."
                ...
                try {
                    Write-Host "Creating fork and cloning into C:\Dev"
                    Set-Location -Path C:\Dev
                    New-ForkRepo -Name $BitBucketUsername.ToLower() -SourceRepository abccicdworkshop/cicd-eshoponweb
                    Write-Host "Success! Created new fork of CI/CD Lab Repo!" -ForegroundColor Green
                }
                catch {
                    $ErrorMessage = $_.Exception.Message
